            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-border">
                        {volumes.map((vol) => (
                            <tr key={vol.name} className="hover:bg-gray-50/60 transition-colors">
                                ...
                                <td className="px-6 py-5">
                                    {editingId === vol.name ? (
                                        <input
                                            ...
                                            className="w-32 rounded-md border border-border bg-surface px-2 py-1.5 text-sm focus:ring-2 focus:ring-brand-100 focus:border-brand-500 outline-none"
                                        />
                                    ) : (
                                ...
                                <td className="px-6 py-5">
                                    {editingId === vol.name ? (
                                        <input
                                            ...
                                            className="w-16 rounded-md border border-border bg-surface px-2 py-1.5 text-sm focus:ring-2 focus:ring-brand-100 focus:border-brand-500 outline-none"
                                        />
                                    ) : (
                                ...
                                <td className="px-6 py-5">
                                    {editingId === vol.name ? (
                                        <select
                                            ...
                                            className="w-40 rounded-md border border-border bg-surface px-2 py-1.5 text-sm focus:ring-2 focus:ring-brand-100 focus:border-brand-500 outline-none min-h-[40px]"
                                        >
                                            {groups.map(g => (
                                ...
                                <td className="px-6 py-5 text-right">
                                    <div className="flex justify-end gap-2 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                        {editingId === vol.name ? (
                                            <>
                                                <button onClick={() => handleSave(vol.name)} className="p-2 rounded-md text-success-600 hover:bg-success-50 transition-colors" title="Save">
                                            ...
                                            <>
                                                <button onClick={() => handleEdit(vol)} className="p-2 rounded-md text-brand-600 hover:bg-brand-50 transition-colors" title="Edit">
                ...
                {volumes.length === 0 && (
                    <div className="py-16 text-center text-sm text-gray-400">No CIFS volumes found</div>
                )}
            </div>
        </div>
    );
